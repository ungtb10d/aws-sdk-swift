...
extension PointInTimeRecoveryDescription: Codable, Reflection {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let earliestRestorableDateTime = earliestRestorableDateTime {
            try container.encode(earliestRestorableDateTime.timeIntervalSince1970, forKey: .earliestRestorableDateTime)
        }
        if let latestRestorableDateTime = latestRestorableDateTime {
            try container.encode(latestRestorableDateTime.timeIntervalSince1970, forKey: .latestRestorableDateTime)
        }
        if let pointInTimeRecoveryStatus = pointInTimeRecoveryStatus {
            try container.encode(pointInTimeRecoveryStatus.rawValue, forKey: .pointInTimeRecoveryStatus)
        }
    }
