...
extension UserType: Codable, Reflection {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let userCreateDate = userCreateDate {
            try container.encode(userCreateDate.timeIntervalSince1970, forKey: .userCreateDate)
        }
        if let userLastModifiedDate = userLastModifiedDate {
            try container.encode(userLastModifiedDate.timeIntervalSince1970, forKey: .userLastModifiedDate)
        }
        if let userStatus = userStatus {
            try container.encode(userStatus.rawValue, forKey: .userStatus)
        }
