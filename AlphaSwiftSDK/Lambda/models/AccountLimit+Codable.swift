...
extension AccountLimit: Codable, Reflection {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if codeSizeUnzipped != 0 {
            try container.encode(codeSizeUnzipped, forKey: .codeSizeUnzipped)
        }
        if codeSizeZipped != 0 {
            try container.encode(codeSizeZipped, forKey: .codeSizeZipped)
        }
        if concurrentExecutions != 0 {
            try container.encode(concurrentExecutions, forKey: .concurrentExecutions)
        }
        if totalCodeSize != 0 {
        ...
        if let unreservedConcurrentExecutions = unreservedConcurrentExecutions {
            try container.encode(unreservedConcurrentExecutions, forKey: .unreservedConcurrentExecutions)
        }
    }
